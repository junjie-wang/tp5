...
trait QueryExtra{
    ...
    public function saveObj(&$data=null, $reGetData=false){
        
        if (empty($data->id)){
            $id = $this->insertGetId((array)$data);
            $data->id=$id;
        }else{
            $id = $data->id;
            $this->where('id',$id)->update((array)$data);
        }
        return true;
    }
    
    public function findObj($data = null){
        $res = $this->find($data);
        return $res?(object)$res:null;
    }
    
//    public function save(&$data=[], $reGetData=false){
...
//    }
    ...
    public function saveObjGet(&$data=null, $reGetData=false){
        if (empty($data->id)){
            $id = $this->insertGetId((array)$data);
            $data->id=$id;
        }else{
...
//            }
//        } else {
//            throw new \Exception('目前暂不支持更新二维数据并返回数据库结果');
//        }
//        return true;
//    }
}
