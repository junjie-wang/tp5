...
class AdminController
{
     ...
     */
    protected function getModel($tablename = '', $type = false)
    {
        if(!$tablename){
             //如果表名不存在则获取当前控制器的名称为表名
             $tablename=$this->request->controller();
        }
        $model = Db::name($this->parseTable($tablename));
        $modelType = 'db';
     ...
     */
    protected function updateField($tablename='',$field, $value, $msg = "操作成功", $pk = "", $input = "")
    {
        if(!$tablename){
            $tablename=$this->request->controller();
        }
